...
def print_info(params):
    ...
    logger.info(f"  {'Equation of State':<34} =  {params.get('eos')}")
    ...
    if params.get('CGL'):
        logger.info(f"  {'Parallel adiabatic index':<34} =  {params.get('parallel_index')}")
        logger.info(f"  {'Perpendicular adiabatic index':<34} =  {params.get('perpendicular_index')}")

    logger.info("Equilibrium parameters:")
    log_head("Current sheet thickness (a)", 'a')
    log_head("Current sheet half-width (w)", 'w')

    logger.info(f"  {'Velocity shear     ':<34} =  {not params.get('noshear')}")

    vmin, vmax, vinc = params.get('vmin'), params.get('vmax'), params.get('vinc')
    is_log = "log10" if params.get('logarithmic') else ""
    ...
    if params.get('sigma_imag') is not None:
        log_head("Imaginary amplitude limit", 'sigma_imag')
    ...
    if mode_val is None:
        mode_val = params.get('modes')
    logger.info(f"  {'Converge the mode':<34} =  {mode_val if mode_val is not None else 'N/A'}")

    logger.info(f"  {'Return all modes':<34} =  {params.get('allmodes')}")
    logger.info(f"  {'Force recalculation':<34} =  {params.get('force')}")
