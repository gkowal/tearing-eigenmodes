...
def modified_case_scale_broadening(
    inner_scale_base: float,
    w: float = 0.0,
    xi: float = 0.0,
    a: float = 1.0,
) -> float:
    """
    Calibrated empirical broadening envelope for modified tearing equilibria with nonzero width (w > 0)
    or normal magnetic field (xi > 0).

    Parameters
    ----------
    inner_scale_base : float
        Theoretical inner scale computed from standard FKR/Coppi asymptotics (w=0, xi=0).
    w : float, optional
        Current sheet width parameter (w >= 0).
    xi : float, optional
        Normal magnetic field strength parameter (xi >= 0).
    a : float, optional
        Current sheet thickness (a > 0).

    Returns
    -------
    float
        Broadened inner scale envelope, guaranteed to be monotone non-decreasing with w and xi.

    Notes
    -----
    - The base FKR/Coppi asymptotics are derived strictly for w = 0 and xi = 0.
    - For w > 0 or xi > 0, numerical convergence tests show that the effective tearing layer
      broadens monotonically. This helper applies a calibrated empirical upper envelope
      factor while retaining the positive theoretical estimate as a conservative fallback.
    """
    ...
    w_hat = w_val / a_val
    xi_hat = xi_val / a_val
    # Monotone non-decreasing envelope (factor == 1.0 when w=0, xi=0)
    factor = 1.0 + 0.5 * np.sqrt(w_hat**2 + xi_hat**2)
    return float(inner_scale_base * factor)


def estimate_modified_inner_scale(
    params: SimulationParams,
    alpha: Optional[float] = None,
) -> float:
    """
    Estimate the inner scale for modified tearing equilibria (w >= 0, xi >= 0).
    ...
    -------
    float
        Estimated inner scale with empirical broadening envelope applied.
    """
    base_scale = estimate_inner_scale(params, alpha=alpha)
    w = params.w if params.w is not None else 0.0
    xi = params.xi if params.xi is not None else 0.0
    a = params.a if params.a is not None else 1.0
    return modified_case_scale_broadening(base_scale, w=w, xi=xi, a=a)
